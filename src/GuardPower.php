...
class GuardPower
{
    ...
    */
    ...
    {
        if (is_dir(__DIR__.'/'.$this->dataFiles)) {
            $files = array('blocked.dat', 'tordatabase.dat', 'cleanedtordatabase.dat');
            foreach ($files as $file) {
                if (file_exists(__DIR__.'/'.$this->dataFiles.'/'.$file)) {
                    //do nothing
                } else {
                    //create file
                    file_put_contents(__DIR__.'/'.$this->dataFiles.'/'.$file, '');
                }
            }
        } else {
            die('Data Directory is not present. Please create a Data directory in the src directory.');
        }
    }
    /**
    * Fetch Data from Tor exit addresses
    */
    public function fetchTorDatabase()
    {
        $update = false;
        $content = file_get_contents(__DIR__.'/'.$this->dataFiles.'/tordatabase.dat');
        if (!empty($content)) {
            //Check last updated
            date_default_timezone_set('UTC');
            $lastModifiedTimestamp = filemtime(__DIR__.'/'.$this->dataFiles.'/tordatabase.dat');
            $time = time();
            $day = $this->diffBtwTimesAsPerType($time, $lastModifiedTimestamp, 3);//Update every 24 hours
            if ($day >= 1) {
                $update = true;
            }
        }
        if ($update) {
            ...
            $handle = fopen('https://check.torproject.org/exit-addresses', 'rb') or die("Could Not Open the specified Tor exit URl. It might be unavailable");
            $write = fopen(__DIR__.'/'.$this->dataFiles.'/tordatabase.dat', 'w+');
            $buffer = "";
            while (!feof($handle)) {
            ...
            $this->prepareTorFile();
            //Remove duplicates
            $this->removeduplicatedlines(__DIR__.'/'.$this->dataFiles.'/cleanedtordatabase.dat', false);
        }
    }
    ...
    */
    ...
    {
        $fp = fopen(__DIR__.'/'.$this->dataFiles.'/tordatabase.dat', "r") or die("Error opening tor database");
        while ($line = fgets($fp)) {
            $explode = explode(" ", $line);
            if (substr_count($explode[1], '.') === 3 && $this->validate_ip($explode[1]) !== false) {
                //Store IP
                $this->fileTypewriter(__DIR__.'/'.$this->dataFiles.'/cleanedtordatabase.dat', $explode[1].PHP_EOL);
            }
        }
        fclose($fp);
    }
    ...
    */
    ...
    {
        $ip = $this->get_ip();
        $fp = fopen(__DIR__.'/'.$this->dataFiles.'/cleanedtordatabase.dat', "r") or die("Error opening tor file");
        while ($line = fgets($fp)) {
            if ($line[0] === $ip) {
                return true;
            }
    ...
    */
    private function request_blocker()
    {
        ...
        $fp = fopen(__DIR__.'/'.$this->dataFiles.'/blocked.dat', "r") or die("Error opening blocked file");
        while ($line = fgets($fp)) {
            $explode = explode(" ", $line);
        ...
        if ($status->getRemainingAttempts() === 0) {
            ...
            $this->fileTypewriter(__DIR__.'/'.$this->dataFiles.'/blocked.dat', $content.PHP_EOL);
            return true;
        }
        return false;
    }
    ...
    */
    private function evaluate_ip()
    {
        ...
        return die('Error: Invalid Address');
    }
    /**
    * IP Normalization
    ...
    * @return string
    */
    private function removeduplicatedlines($filepath, $ignorecase= false, $newline= PHP_EOL)
    {
        $content = file_get_contents($filepath);
        $content = $this->removeduplicatedlinesbystring($content, $ignorecase, $newline);
        // write the new file
     ...
     * @return string
     */
    private function removeduplicatedlinesbystring($lines, $ignorecase= false, $newline = PHP_EOL)
    {
        if (is_array($lines)) {
